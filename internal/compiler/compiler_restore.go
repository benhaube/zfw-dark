...
func CompileRestoreScript(rs rules.RuleSet, dockerPorts map[int]bool, geoFiles map[string]string, extraBypass ...string) string {
	...
	b.WriteString(`IPTR="$(command -v iptables-legacy-restore 2>/dev/null || command -v iptables-restore)"` + "\n")
	b.WriteString(`IPT6="$(command -v ip6tables-legacy 2>/dev/null || command -v ip6tables 2>/dev/null || true)"` + "\n")
	b.WriteString(`IPT6R="$(command -v ip6tables-legacy-restore 2>/dev/null || command -v ip6tables-restore 2>/dev/null || true)"` + "\n")
