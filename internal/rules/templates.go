...
func Templates(lan string) []Template {
	...
	return []Template{
		...
		{
			ID:          "allow-arr-stack",
			Name:        "Allow *arr stack (Sonarr / Radarr / Lidarr / Readarr / Bazarr / Prowlarr)",
			Description: "Allow the full six-app media-management *arr stack on default ports: Sonarr 8989, Radarr 7878, Lidarr 8686, Readarr 8787, Bazarr 6767, Prowlarr 9696. One template add covers the canonical homelab automation suite — pair with qBittorrent / SABnzbd as the actual downloader.",
			Category:    "service",
			Rules:       []Rule{allow("Allow *arr stack", "tcp", 8989, 7878, 8686, 8787, 6767, 9696)},
		},
		{
			ID:          "allow-qbittorrent",
			Name:        "Allow qBittorrent (8080)",
			Description: "Allow the qBittorrent web UI on its canonical default port. NOTE: 8080 also defaults for Nextcloud AIO, Dozzle and OpenWebUI — if multiple of those run on the same host you'll need to remap one (qBittorrent supports `-c` or env-var override).",
			Category:    "service",
			Rules:       []Rule{allow("Allow qBittorrent", "tcp", 8080)},
		},
		{
			ID:          "allow-paperless-ngx",
			Name:        "Allow Paperless-NGX (8000)",
			Description: "Allow the Paperless-NGX self-hosted document-management app on its default web port. Standard ZimaOS Mod-Store entry; pair with the consumer-folder watcher to scan-and-upload from your devices on the LAN.",
			Category:    "service",
			Rules:       []Rule{allow("Allow Paperless-NGX", "tcp", 8000)},
		},
		{
			ID:          "allow-mealie",
			Name:        "Allow Mealie (9000)",
			Description: "Allow the Mealie self-hosted recipe-management app on its default web port. NOTE: 9000 also defaults for Portainer — if both apps run on the same host you'll need to remap one.",
			Category:    "service",
			Rules:       []Rule{allow("Allow Mealie", "tcp", 9000)},
		},
		{
			ID:          "allow-dozzle",
			Name:        "Allow Dozzle (8080)",
			Description: "Allow the Dozzle live-Docker-log web UI on its default port. NOTE: 8080 conflicts with qBittorrent / Nextcloud AIO / OpenWebUI defaults — remap one of them if collocated; Dozzle accepts `--port` or `DOZZLE_BASE` to move.",
			Category:    "service",
			Rules:       []Rule{allow("Allow Dozzle", "tcp", 8080)},
		},
		{
			ID:          "allow-n8n",
			Name:        "Allow n8n (5678)",
			Description: "Allow the n8n workflow-automation web UI on its default port. Typical ZimaOS Mod-Store entry — opens the editor from your other LAN devices without needing a tunnel.",
			Category:    "service",
			Rules:       []Rule{allow("Allow n8n", "tcp", 5678)},
		},
		{
			ID:          "allow-openwebui",
			Name:        "Allow OpenWebUI (3000)",
			Description: "Allow the OpenWebUI (Ollama / LLM frontend) web UI on its canonical Docker-Compose default port (host:3000 → container:8080). NOTE: 3000 also defaults for AdGuard Home's initial setup — those scenarios are typically not concurrent, but remap if needed.",
			Category:    "service",
			Rules:       []Rule{allow("Allow OpenWebUI", "tcp", 3000)},
		},
	}
}
